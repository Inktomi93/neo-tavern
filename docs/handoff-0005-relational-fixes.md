# Handoff — Migration 0005: relational integrity + preset versioning
...
4. **Hand-written SQL must survive.** Migration `0001` hand-adds the `libsql_vector_idx` ANN index (drizzle-kit can't emit it). **0005 is additive — do not regenerate/rewrite 0000–0004.** Hand-read the generated 0005 SQL before applying; drizzle's SQLite differ sometimes recreates more than intended — confirm it does not drop the `message_variants`/import columns from 0002/0003 or the vector column/index.
...
- `src/db/schema.ts` — schema. `src/db/migrations/` — 0000–0004; add **0005**. `src/db/client.ts` — PRAGMAs + `runMigrations`.
