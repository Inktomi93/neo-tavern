...
+ the one short backlog in `docs/planning/build-plan.md`. For *how to work* see **`AGENTS.md`**;
for the enforced architecture see **`docs/architecture/architecture.md`**.
...
- **Corpus RAG:** we DO use the homelab's 2× RTX A6000. **BGE-M3** (1024-dim, CLS+normalize)
  ...
  **vendored project-locally with uv** (`tools/cuda/`, `pnpm cuda:setup`). Real token counts
  ...
  away. Details: `docs/subsystems/corpus-import.md`.
...
**`docs/architecture/` — the enforced rails (read before touching imports):**
- **`architecture.md`** — the enforced layer cake + folder map.
- **`conventions.md`** — recurring tooling/lint/logging traps (READ before fighting Biome/tsc).
- **`data-model.md`** — schema design notes (the *why*). The schema *itself* lives in `src/db/schema/`
  (heavily commented, aggregated by `src/db/schema.ts` — that's the source of truth, not a parallel spec).
- **`dependencies.md`** — deps (installed + deferred parking lot).
- **`dependency-graph.mmd`** — the live module graph (generated by `pnpm arch:graph`).

**`docs/auth/` — auth + credentials:**
- **`auth-and-credentials-plan.md`** — the auth/credentials design (BUILT): pluggable `AUTH_MODE`,
  cookie/BFF sessions, the credential resolver, per-user encrypted keys. The *why*.
- **`auth-deploy.md`** — the operational recipe: the paste-ready Caddy block + authentik checklist + env. *How to deploy.*
- **`auth-verify.md`** — the verification runbook: `pnpm verify:auth` + the live walkthrough. *How to confirm it works.*

**`docs/subsystems/` — per-feature deep dives:**
- **`corpus-import.md`** — the ST import + RAG answer key (parsers/ranking lifted from card-curator & st-bridge).
- **`sdk-notes.md`** — provider SDK field notes: Agent SDK (runtime, event matrix, compaction, seeding, caching) + OpenRouter.
- **`chat-memory.md`** (+ **`chat-memory-diagram.md`**) — the within-chat `{{memory}}` system: structured
  digests, tiering, hybrid corpus search, CSLS. The visual companion is the `-diagram` file.
- **`assets.md`** — the content-addressed blob store (CAS) design + the caddy serving contract.
- **`observability.md`** — pino logging + the `curl`-able `/api/_debug/*` API.
- **`settings.md`** — the three typed config tiers (env / AppSettings / UserSettings); the as-built surface.

**`docs/planning/` — backlog, specs, audits:**
- **`build-plan.md`** — the de-risk spike results + the **deferred backlog** (what's left).
- **`breadth-buildout.md`** — the implement-from-cold spec for three breadth tracks: security (SSRF +
  rate-limit + hardening), corpus RAG analytics (co-occurrence/themes/dedup), full type-safety. Pick a track.
- **`ui-direction.md`** — the client UX plan (the next big chunk of work).
- **`parity-audit.md`** — the live, hand-verified SillyTavern-vs-neo backend audit (inventory from
  `pnpm api:q`, verdicts manual). Outstanding gaps + close-the-gap plans.
- **`maintenance-and-scheduling.md`** — the decided-not-built seam for scheduled work: one `maintenance`
  registry + `scheduled_runs` table + croner, executor-swappable. The *why*.

**`docs/ops/` — deploy/runbooks:**
- **`cloud-setup.md`** — running neo-tavern in Claude Code on the web (setup script + SessionStart hook + env).
...
- **`references/README.md`** — reference repos (read, don't copy). · **`README.md` / `ONBOARDING.md`** — how to run / onboarding.
...
the **deferred backlog in `docs/planning/build-plan.md`**. The shape as of this writing: rails + the
...
- **Vectors are libSQL NATIVE `F32_BLOB` + `libsql_vector_idx`** — NO sqlite-vec. The ANN index is
  ...
  (`docs/architecture/conventions.md`, `corpus-import.md`)
...
- **Internal links are enforced FKs** (migration 0007 — cascade policy in `docs/architecture/data-model.md`);
...
- **Generation knobs are ONE provider-agnostic vocab** (`GenerationParams`, `shared/generation.ts`,
  ...
  model-gated (`xhigh`=Opus 4.7, `max`=Opus 4.6/4.7+Sonnet 4.6). (`docs/subsystems/sdk-notes.md`)
- **Time is epoch-ms UTC EVERYWHERE.** `shared/time.ts` (Luxon) is the only parser; normalize at
  ...
  string we emit is the Agent SDK's session-frame `timestamp` (its shape). (`docs/architecture/conventions.md`)
...
- **`max-pro-sub` is the OWNER's single host credential** (the `claude login` Max sub). Gated by ONE
  ...
  `docs/auth/auth-and-credentials-plan.md` §8 chokepoint) — the chat verbs (send/swipe/generateOpening/
...
- **Three typed config tiers** (`docs/subsystems/settings.md`): `env.ts` (deploy/box/secret/identity —
