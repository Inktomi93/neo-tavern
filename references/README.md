...
git clone --depth 1 https://github.com/Samueras/GuidedGenerations-Extension references/guided-generations
# ST memory extensions — the answer-keys behind docs/subsystems/chat-memory.md §11:
git clone --depth 1 https://github.com/bal-spec/sillytavern-character-memory references/sillytavern-character-memory
git clone --depth 1 https://github.com/aikohanasaki/SillyTavern-MemoryBooks references/SillyTavern-MemoryBooks
git clone --depth 1 https://github.com/Lodactio/Extension-Summaryception references/Extension-Summaryception
git clone --depth 1 https://github.com/qvink/SillyTavern-MessageSummarize references/SillyTavern-MessageSummarize
...
| **memory extensions** (`sillytavern-character-memory` · `SillyTavern-MemoryBooks` · `Extension-Summaryception` · `SillyTavern-MessageSummarize`) | clone | The **chat-memory answer-keys** ([docs/subsystems/chat-memory.md](../docs/subsystems/chat-memory.md) §11): CharMemory → topic-anchor + significance litmus; MemoryBooks → LLM-extracted keywords as a retrieval key; Summaryception → consolidation-only delta prompting; qvink (MessageSummarize) → manual per-digest edit + pre-summarization noise filtering. Read the *concepts*; we re-express them on our digest schema. |
...
| **card-curator** | symlink | **Our** Python ST corpus tool — the deep RAG **answer key**: PNG card + chat parsers, CSLS hubness, segmentation, field budgeting. Lift `file:line` per `docs/subsystems/corpus-import.md`. |
