...
Two providers are wired (adapters in `src/server/providers/`) and consumed by the
sdk-mode chat turn (`domain/chat` → `runChatTurn`, which logs model/cost/latency metadata).
...
The DB schema (+ migrations 0000–0007, incl. enforced FKs + preset versioning), the
sdk-mode chat turn, the full corpus RAG/search stack (BGE-M3 embeddings, CSLS hubness,
bge-reranker-v2-m3 two-stage rerank, `discover`), the `/corpus` search UI, and the
ST importer are all built + validated. See `CLAUDE.md` build-phases + `docs/build-plan.md`
for status. **Still deferred:** Docker/compose, Playwright E2E, the mode escape valve
(Phase 5), and analytics charts (Phase 6).
